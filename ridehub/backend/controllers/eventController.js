...
export const getEvent = async (req, res) => {
  try {
    ...
    const event = await Event.findById(eventId).populate("creator"); // Suponiendo que tienes un modelo 'Event'
    if (!event) {
...
export const commentOnEvent = async (req, res) => {
  ...
  try {
    ...
    await event.save();
    await event.populate("comments.user", "username photo"); // Poblamos el usuario del comentario
